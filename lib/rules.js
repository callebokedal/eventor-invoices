...
function _calculateDiscount(text, age, amount, lateFee) {
	...
	const openClass = /öppna klasser/ig;
	const specificDiscount = /stafett|veteran|kavlen/ig;
	if(specificDiscount.test(text) && !openClass.test(text)) {
	...
	const publiktavling = /publiktävling/ig;
	if(/sm, |sm sprint/ig.test(text) && !publiktavling.test(text)) {
	  return {amount:amount, percent: 100}
	}
